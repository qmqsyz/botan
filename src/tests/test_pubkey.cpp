...
class PK_API_Sign_Test : public Text_Based_Test {
   ...
   protected:
      Test::Result run_one_test(const std::string& header, const VarMap& vars) final {
         ...
         // Test::Result result(Botan::fmt("Sign/verify API tests {}({})/", algorithm, algo_params, sig_params));
         std::ostringstream test_name;
         test_name << "Sign/verify API tests " << algorithm << '(' << algo_params << ')';
         if(!sig_params.empty()) {
         ...
         pubkey.reset();
         privkey.reset();
         // TODO: reuse message/signature, as some raw operations have specific requirements
